        ...
        if (waWebKnowledgeBase && waWebKnowledgeBase.autoReplyEnabled) {
          m.messages.forEach(async (msg) => {
            try {
              ...
              setTimeout(async () => {
                try {
                  // 🧠 CONTACT BRAIN: per-contact control before anything is generated or sent
                  let brain = null;
                  try { brain = await getBrainForReply(remoteJid, resolveContactName(remoteJid)); } catch (e) { /* non-fatal */ }
                  if (brain && (brain.mode === 'silent' || brain.mode === 'alert_only')) {
                    await saveBrainDraft({
                      jid: remoteJid, name: brain.name, kind: 'alert',
                      incoming: (transcribedAudioText || text || '').trim() || '[' + (mediaType || 'message') + ']',
                      reason: brain.mode
                    });
                    console.log('[WA-WEB BRAIN] 🧠 ' + brain.mode + ' for ' + remoteJid + ' - alert saved, NO auto-reply sent');
                    return;
                  }
                  ...
                  // Escalation: the brain says hands-off — save alert + WhatsApp alert to the owner, never send to the contact
                  if (/^ESCALATE\b/i.test(finalReply)) {
                    await escalateBrain({ jid: remoteJid, name: brain && brain.name, detail: (transcribedAudioText || text || '').trim() || '[media]' });
                    console.log('[WA-WEB BRAIN] 🚨 ESCALATE for ' + remoteJid + ' - no reply sent, owner alerted');
                    return;
                  }
                  // Draft mode: save the reply for approval; nothing leaves the phone
                  if (brain && brain.mode === 'draft_only') {
                    await saveBrainDraft({
                      jid: remoteJid, name: brain.name, kind: 'draft',
                      incoming: (transcribedAudioText || text || '').trim() || '[' + (mediaType || 'message') + ']',
                      draft: finalReply, reason: 'draft_only'
                    });
                    console.log('[WA-WEB BRAIN] 📥 Draft saved for approval (' + remoteJid + ') - nothing sent');
                    return;
                  }
                  // Human-feel delay + typing indicator (optional per contact)
                  if (brain && brain.humanDelay) {
                    try { if (sock && waWebState.status === 'connected') await sock.sendPresenceUpdate('composing', remoteJid); } catch (e) { /* presence optional */ }
                    const waitMs = (4 + Math.floor(Math.random() * 9)) * 1000; // 4-12s
                    await new Promise(r => setTimeout(r, waitMs));
                  }
                  await sendWaWebMessage(remoteJid, finalReply);
                  console.log('[WA-WEB AUTO-REPLY] 🟢 Successfully Auto-replied to ' + remoteJid + ': ' + finalReply);
                } catch (e) {
...
// 🧠 Contact Brain support: recent messages (memory-first, vault fallback) for style
// learning + the dashboard AI discussion area. Returns newest-last, capped.
export async function collectBrainChatMessagesForLearning(jid, maxMessages = 120) {
  const chat = waWebState.chats.get(jid);
  if (!chat) return [];
  if (chat._messagesLoaded === false) {
    try { await hydrateChatFromVault(jid); } catch (e) { /* keep whatever we have */ }
  }
  const all = (chat.messages || []).slice().sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));
  return all.slice(-Math.max(1, Math.min(maxMessages, 500)));
}
...
'Task: Compose a natural, professional WhatsApp reply following all business rules. If greeting, address by their name. Do not repeat greeting if already in conversation.');
    ...
    if (!overridePrompt) {
      try {
        const brainSection = await brainPromptSectionForJid(jid);
        if (brainSection) {
          systemPrompt += brainSection;
          console.log('[WA-WEB BRAIN] 🧠 Brain rules applied for ' + jid);
        }
      } catch (e) { console.warn('[WA-WEB BRAIN] prompt injection failed:', e.message); }
    }
